...
def bfgs(lambdified_fn, lambdified_grad, initial_x, line_search_fn, max_iter = 5000, initial_alpha = 1):
	x_values, obj_values, alphas = [initial_x], [lambdified_fn(*initial_x)], [None]
	curr_iter = 0
	n = len(initial_x)
	B = [[1 if i == j else 0 for i in range(n)] for j in range(n)]
	ones_matrix = [[1 for i in range(n)] for j in range(n)]
	while curr_iter < max_iter:
		...
		direction = -np.matmul(evaluated_grad, B)
		...
		first_term = (ones_matrix + (gTBg / pTg)) * (ppT / pTg)
		second_term = (pgTB + BgpT) / gTp

		B = B + first_term - second_term
